    ...
    for index_set in cand_indices:
        ...
        for index in index_set:
            ...
            if random() < 0.8:
                if random()<0.3:
                    masked_token = "[MASK]"
                    for s_len in range(3):
                        masked_lms.append(MaskedLmInstance(index=index+s_len, label=tokens[index+s_len]))
                        tokens[index+s_len] = masked_token
                elif random()<0.4:
                    masked_token = "[MASK]"
                    for s_len in range(2):
                        masked_lms.append(MaskedLmInstance(index=index+s_len, label=tokens[index+s_len]))
                        tokens[index+s_len] = masked_token
                else:
                    masked_token = "[MASK]"
                    for s_len in range(1):
                        masked_lms.append(MaskedLmInstance(index=index+s_len, label=tokens[index+s_len]))
                        tokens[index+s_len] = masked_token
